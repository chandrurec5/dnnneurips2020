...
Ours &$G_{sr}(q)=\frac{1}{1+\exp(-\beta q)}$ &$\chi_{sr}(q)=q\cdot G_{sr}(q)$& $\dot{\chi}_{sr}(q)=\dot{\chi}_{sr}(q)+\underbrace{\dot{\gamma}_{sr}(q)}_{\text{Gate Dynamics}}$\\\hline
\end{tabular}
}
