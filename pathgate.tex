...
Path& $p\stackrel{def}=(p(0),p(1),\ldots,p(d))$\\\hline
\emph{Value} &$v_t(p)\stackrel{def}=\Pi_{l=1}^d \Theta_t(l,p(l-1),p(l))$ \\\hline
\emph{Activation}& $A_{t}(x,p)\stackrel{def}{=}\Pi_{l=1}^{d-1} G_{x,t}(l,p(l))$\\\hline
