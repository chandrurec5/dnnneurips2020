...
\textbf{Gating:} In what follows, we consider two kinds of gates namely hard and soft gates. For a pre-activation input $q\in\R$, the  hard and soft gates are given by $\mathbbm{1}_{\{\q>0\}}$ and $\frac{1}{1+\exp(-\beta q)}$, where $\beta>0$ is a positive constant. Using the hard and soft gates, we can specify the ReLU and a `soft-ReLU' activations as $q\mathbbm{1}_{\{q>0\}}$ and $q\frac{1}{1+\exp(-\beta q)}$.\\
