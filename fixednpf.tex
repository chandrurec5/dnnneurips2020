...
A comparison of the GD dynamics across the three regimes is presented in \Cref{tb:dynamics} and \Cref{fig:regime}. In the standard regime, width is finite, and the NPFs are learnt. Practical DNNs with ReLU activations operate in the standard regime. In the `large-width' regime [\citenum{arora2019exact}], the NPFs and the NTFs do not deviate much from initialisation.

 Note that if we let $w\ra\infty$ in the fixed NPF regime, can have a regime which is both `large-width' as well as fixed NPF (see middle region in \Cref{fig:regime}). 
\FloatBarrier
