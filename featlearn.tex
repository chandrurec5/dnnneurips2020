...
\begin{theorem}\label{th:main} Let $\delta_t(s,s')\stackrel{def}=\sum_{p\rsa i} \ip{\varphi^a_{x_s,p,t},\varphi^a_{x_{s'},p,t}}$, for $s,s'\in[n]$, using any $i\in[d_{in}]$. Then, under \Cref{assmp:main}, we have:\\
...
\end{theorem}
\textbf{Discussion:}We hypothesise the following:\\
...
(ii) Subject to the `well-conditioned' ness of $K^{\phi}_t$, such margin increase can be perhaps achieved for all the $n$ examples. \\
